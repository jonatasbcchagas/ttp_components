...
/*      
   ...
   MAX_2D               Weights are maximum distances in 3-D
   ...
   CEIL-3D              Weights are Euclidean distances in 2-3 rounded up
...
void ReadProblem()
{
    ...
    assert(SwapStack = (SwapRecord *) malloc((Dimension + 10) * sizeof(SwapRecord)));
    ...
    if (CostMatrix == 0 && Dimension <= MaxMatrixDimension && Distance != 0 &&
        ...
        Node *Ni, *Nj;
        assert(CostMatrix = (long *) calloc(Dimension * (Dimension - 1) / 2, sizeof(long)));
        Ni = FirstNode->Suc;
        do {
...
void CheckSpecificationPart()
{
    ...
    if (WeightType != EXPLICIT && WeightType != SPECIAL && WeightFormat != -1)
...
static void Read_DISPLAY_DATA_SECTION()
{
    ...
    for (i = 1; i <= Dimension; i++) {
        ...
        if (Id <= 0 || Id > Dimension)
            eprintf("(DIPLAY_DATA_SECTION) Node number out of range: %ld", Id);
        N = &NodeSet[Id];
        if (N->V == 1)
            eprintf("(DIPLAY_DATA_SECTION) Node number occours twice: %ld", N->Id);
        N->V = 1;
    ...
    while ((N = N->Suc) != FirstNode);
    if (!N->V)
        eprintf("(DIPLAY_DATA_SECTION) No coordinates given for node %ld", N->Id);
    if (ProblemType == HPP)
        Dimension++;
}
...
static void Read_EDGE_DATA_SECTION()
{
    ...
    if (!strcmp(EdgeDataFormat, "EDGE_LIST")) {
        ...
        while (i != -1) {
            if (i <= 0 || i > Dimension)
                eprintf("(EDGE_DATA__SECTION) Node number out of range: %ld", i);
            fscanf(ProblemFile, "%ld\n", &j);
            if (j <= 0 || j > Dimension)
                eprintf("(EDGE_DATA_SECTION) Node number out of range: %ld", j);
            if (i == j)
                eprintf("(EDGE_DATA_SECTION) Illgal edge: %ld to %ld", i, j);
            Ni = &NodeSet[i];
            Nj = &NodeSet[j];
            if (!Ni->CandidateSet) {
                Ni->V = 1;
...
static void Read_EDGE_WEIGHT_FORMAT()
{
    ...
    if (!strcmp(EdgeWeightFormat, "FUNCTION"))
        eprintf("EDGE_WEIGHT_FORMAT not implemented: %s", EdgeWeightFormat);
...
static void Read_FIXED_EDGES_SECTION()
{
    ...
    while (i != -1) {
        if (i <= 0 || i > Dimension)
            eprintf("(FIXED_EDGES_SECTION) Node number out of range: %ld", i);
        fscanf(ProblemFile, "%ld\n", &j);
        if (j <= 0 || j > Dimension)
            eprintf("(FIXED_EDGES_SECTION) Node number out of range: %ld", j);
        if (i == j)
...
static void Read_NODE_COORD_SECTION()
{
    ...
    for (i = 1; i <= Dimension; i++) {
        ...
        if (Id <= 0 || Id > Dimension)
            eprintf("(NODE_COORD_SECTION) Node number out of range: %ld", Id);
        N = &NodeSet[Id];
        if (N->V == 1)
            eprintf("(NODE_COORD_SECTION) Node number occours twice: %ld", N->Id);
        N->V = 1;
        if (!fscanf(ProblemFile, "%lf", &N->X))
...
static void Read_NODE_COORD_TYPE()
{
    ...
    if (!(NodeCoordType = Copy(strtok(0, Delimiters))))
        eprintf("(EDGE_COORD_TYPE): string expected");
    for (i = 0; i < strlen(NodeCoordType); i++)
        NodeCoordType[i] = (char) toupper(NodeCoordType[i]);
    if (strcmp(NodeCoordType, "TWOD_COORDS") != 0)
        CoordType = TWOD_COORDS;
    else if (strcmp(NodeCoordType, "THREED_COORDS") != 0)
        CoordType = THREED_COORDS;
    else if (strcmp(NodeCoordType, "NO_COORDS") != 0)
        CoordType = NO_COORDS;
    else
        eprintf("Unknown EDGE_COORD_TYPE: %s", NodeCoordType);
}
...
static void Read_TOUR_SECTION(FILE * File)
{
    ...
    for (k = 0; k < Dimension && i != -1; k++) {
        ...
        N = &NodeSet[i];
        if (N->V == 1)
            eprintf("(TOUR_SECTION) Node number occours twice: %ld", N->Id);
        N->V = 1;
        if (k == 0)
            First = Last = N;
        else {
    ...
    if (Last)
        Last->OptimumSuc = ProblemType != ATSP ? First : First->OptimumSuc;
    N = FirstNode;
    do
        if (!N->V)
            break;
...
/*
   The ReadInputTour function reads a tour from a file, if requested in the parameter 
   file (by using the INPUT_TOUR keyword). The tour is used to limit the search
...
void ReadInputTour()
{
    ...
    if (!(InputTourFile = fopen(InputTourFileName, "r")))
        eprintf("Cannot open %s", InputTourFileName);
    while (Line = ReadLine(InputTourFile)) {
        if (!(Keyword = strtok(Line, Delimiters)))
            continue;
        for (i = 0; i < strlen(Keyword); i++)
            Keyword[i] = (char) toupper(Keyword[i]);
        if (!strcmp(Keyword, "COMMENT") ||
            ...
            Read_TOUR_SECTION(InputTourFile);
        else if (strcmp(Keyword, "EOF") == 0)
            break;
        else
            eprintf("Unknown Keyword: %s", Keyword);
    }
    fclose(InputTourFile);
}

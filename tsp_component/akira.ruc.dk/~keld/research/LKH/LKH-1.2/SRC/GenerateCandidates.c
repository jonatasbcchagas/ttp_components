...
/*
   ...
   Keld Helsgaun,
   An Effective Implementation if the Lin-Kernighan Traveling Salesman Heuristic,
   Report, RUC, 1998. 
 */
...
void GenerateCandidates(const long MaxCandidates, const long MaxAlpha, const int Symmetric)
{
    ...
    do {
        assert(From->CandidateSet =
          (Candidate *) malloc((MaxCandidates + 1) * sizeof(Candidate)));
    }
    while ((From = From->Suc) != FirstNode);
    ...
    do {
        ...
        if (From != FirstNode) {
            ...
            for (To = From; To->Dad != 0; To = To->Dad) {
                To->Dad->Beta = !Fixed(To, To->Dad) ? max(To->Beta, To->Cost)
                    : To->Beta;
                To->Dad->Mark = From;
            }
        }
        ...
        do {
            ...
            if (From == FirstNode)
                ...
                if (To->Mark != From)
                    To->Beta = !Fixed(To, To->Dad) ? max(To->Dad->Beta, To->Cost)
                        : To->Dad->Beta;
                a = d - To->Beta;
            }
            if (Fixed(From, To))
                ...
                if (InOptimumTour(From, To)) {
                    ...
                    if (a > MaxAlpha ||
                        (Count == MaxCandidates &&
                         (a > (NFrom - 1)->Alpha ||
                    (a == (NFrom - 1)->Alpha && d >= (NFrom - 1)->Cost))))
                        continue;
                    if (To == From->Dad) {
                        d = From->Cost;
                        a = 0;
    ...
    do {
        for (NTo = To->CandidateSet; From = NTo->To; NTo++) {
            Count = 0;
            for (NN = NFrom = From->CandidateSet; NN->To && NN->To != To; NN++)
                Count++;
            if (!NN->To) {
                a = NTo->Alpha;
                d = NTo->Cost;

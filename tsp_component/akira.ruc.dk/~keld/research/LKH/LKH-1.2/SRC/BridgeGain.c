        ...
        for (Nt2 = t2->CandidateSet; t3 = Nt2->To; Nt2++) {
            ...
            for (X4 = 1; X4 <= 2; X4++) {
                ...
                /* If BridgeGain has been called with a nonfeasible 2-opt move,
                   then try to find a 3-opt or 4-opt move which, when omposed with 
                ...
                for (Nt4 = t4->CandidateSet; t5 = Nt4->To; Nt4++) {
                    ...
                    /* Choose t8 as a candidate edge emanating from t7.
                       Only one choice! */
                    for (Nt6 = t6->CandidateSet; t7 = Nt6->To; Nt6++) {
                        ...
                        if (X4 == 1)
                            t8 = (BETWEEN(u2, t5, t1) ? BETWEEN(t5, t7, t1) :
                                  BETWEEN(t2, t5, u3) ? BETWEEN(u2, t7, t1) || BETWEEN(t5, t7, u3) :
                                  BETWEEN(SUC(u3), t5, t3) ? BETWEEN(u2, t7, u3) || BETWEEN(t5, t7, t3) :
                              !BETWEEN(t4, t7, t6)) ? PRED(t7) : SUC(t7);
                        else
                            t8 = (BETWEEN(u2, t5, t1) ? !BETWEEN(u2, t7, t6) &&
                                  !BETWEEN(t2, t7, u3) :
                             BETWEEN(t2, t5, u3) ? !BETWEEN(t2, t7, t6) :
                                  BETWEEN(SUC(u3), t5, t4) ? !BETWEEN(SUC(u3), t7, t5) &&
                                  !BETWEEN(t3, t7, PRED(u2)) :
                              !BETWEEN(t3, t7, t5)) ? PRED(t7) : SUC(t7);
                        if ((t7 == t1 && t8 == t2) ||
                            (t7 == t2 && t8 == t1) ||
                            (t7 == t3 && t8 == t4) ||
                            (t7 == t4 && t8 == t3) ||
